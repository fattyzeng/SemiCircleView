...
public class SemiCircleMenu extends View {
	...
	private HashMap<String, SemiCircleMenuItem> mMenuItems = new HashMap<String, SemiCircleMenuItem>();
	...
	protected void onDraw(Canvas canvas) {
		...
		if(isMenuVisible) {
			canvas.drawArc(mMenuRect, mStartAngle, 360, true, mRadialMenuPaint);
			...
			if(mMenuItems.size() > 0) {
				...
				for(SemiCircleMenuItem item : mMenuItems.values()) {
		...
		canvas.drawArc(mMenuCenterButtonRect, mStartAngle, 360, true, mRadialMenuPaint);
		mRadialMenuPaint.setShadowLayer(mShadowRadius, 0.0f, 0.0f, Color.TRANSPARENT);  
	...
	/**
	 * this function that do sth when you presses down 
	 */
	...
	/**
	 * this function that do sth when you presses up 
	 */
	private void ActionDown(int x, int y) {
		if(mMenuCenterButtonRect.contains(x, y)) {
			centerRadialColor = SemiCircleMenuColors.HOLO_LIGHT_BLUE;
	...
	private void drawCenterText(Canvas canvas, Paint paint) {
		...
		case HORIZONTAL_BOTTOM:
			canvas.drawText(centerMenuText, (getWidth()/2)-paint.measureText(centerMenuText)/2, getHeight()/2, paint);
			break;
		}
	}
	...
	private RectF getRadialMenuRect(boolean isCenterButton) {
		...
		case VERTICAL_RIGHT:
			...
			left = getWidth() - (int) (mRadius/scale);
			right = getWidth() + (int) (mRadius/scale);
			top = (getHeight()/2) - (int) (mRadius/scale);
			bottom = (getHeight()/2) + (int) (mRadius/scale);
			mStartAngle = 90;
			mViewAnchorPoints = new Point(getWidth(), getHeight()/2);
			break;
		case VERTICAL_LEFT:
			left = -(int) (mRadius/scale);
			right = (int) (mRadius/scale);
			top = (getHeight()/2) - (int) (mRadius/scale);
			bottom = (getHeight()/2) + (int) (mRadius/scale);
			mStartAngle = 270;
			...
			break;
		case HORIZONTAL_TOP:
			left = (getWidth()/2) - (int) (mRadius/scale);
			right = (getWidth()/2) + (int) (mRadius/scale);
			top = -(int) (mRadius/scale);
			bottom = (int) (mRadius/scale);			
			mStartAngle = 0;
		...
		case HORIZONTAL_BOTTOM:
			...
			bottom = getHeight()/2 + (int) (mRadius/scale);
			mStartAngle = 0;
			mViewAnchorPoints = new Point(getWidth()/2, getHeight()/2);
			break;
		}
	 ...
	 */
	public void addMenuItem(String idTag, SemiCircleMenuItem mMenuItem) {
